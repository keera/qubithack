...
var tokenize = function(str) {
  var matches = [];
  var tick = function(len) {
     str = str.slice(len);
  };
  var eatTok = function(name, val) {
    matches.push(new Token(name, val));
    tick(val.length);
  };
  while (str.length) {
    var hasMatch = false;
...
Matcher.prototype.searchElements = function(type, value) {
  ...
  switch (type) {
    case "id": this.matcher = this.idMatcher; break;
    case "tag": this.matcher = this.tagMatcher; break;
    case "class": this.matcher = this.classMatcher; break;
  }
...
var matcher = new Matcher(document);
// Parser
var getTok = function() {
  return toks.shift();
};
...
// Here is where we set our predicates dictated by the combinator
...
var $ = function (selector) {
  matcher = new Matcher(document);
  toks = tokenize(selector);
